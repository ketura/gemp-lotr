...
public class Card4_157 extends AbstractAttachable {
    ...
    public boolean checkPlayRequirements(String playerId, LotroGame game, PhysicalCard self, Filter additionalAttachmentFilter, int twilightModifier) {
        return super.checkPlayRequirements(playerId, game, self, additionalAttachmentFilter, twilightModifier)
                && PlayConditions.canExert(self, game, Filters.or(Filters.saruman, Filters.and(Filters.culture(Culture.ISENGARD), Filters.race(Race.MAN))));
    }
    ...
    public AttachPermanentAction getPlayCardAction(String playerId, LotroGame game, PhysicalCard self, Filter additionalAttachmentFilter, int twilightModifier) {
        ...
        playCardAction.appendCost(
                new ChooseAndExertCharactersEffect(playCardAction, playerId, 1, 1, Filters.or(Filters.saruman, Filters.and(Filters.culture(Culture.ISENGARD), Filters.race(Race.MAN)))));
