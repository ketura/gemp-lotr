package com.gempukku.lotro.cards.set21.elven;
 ...
 */
public class Card21_21 extends AbstractAlly {
    public Card21_21() {
    ...
    public List<OptionalTriggerAction> getOptionalAfterTriggers(final String playerId, LotroGame game, EffectResult effectResult, final PhysicalCard self) {
        if (TriggerConditions.startOfTurn(game, effectResult)) {
            ...
            action.appendCost(
                    new ChooseActiveCardEffect(self, playerId, "Choose a Wise ally", Filters.and(CardType.ALLY, keyword(Keyword.WISE)), Filters.canHeal) {
                @Override
                protected void cardSelected(LotroGame game, final PhysicalCard card) {
                    action.appendEffect(
							new ChooseAndHealCharactersEffect(action, playerId, 1, 1, Filters.name(getName()) ? 2 : 1, Filters.and(CardType.ALLY, keyword(Keyword.WISE))));
					return action;;
                }
            });
