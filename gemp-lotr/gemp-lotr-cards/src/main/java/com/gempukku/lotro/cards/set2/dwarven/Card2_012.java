...
public class Card2_012 extends AbstractPermanent {
    public Card2_012() {
        super(Side.FREE_PEOPLE, 1, CardType.CONDITION, Culture.DWARVEN, "Realm of Dwarrowdelf", null, true);
    }
            ...
            action.appendEffect(
                    new ChooseActiveCardEffect(self, playerId, "Choose a Dwarf", Race.DWARF) {
                        ...
                        protected void cardSelected(LotroGame game, PhysicalCard dwarf) {
                            action.appendEffect(
                                    new CheckPhaseLimitEffect(action, self, dwarf.getCardId()+"-str", 3, Phase.SKIRMISH,
                                            new AddUntilEndOfPhaseModifierEffect(
                                                    new StrengthModifier(self, Filters.sameCard(dwarf), 1))));
                            if (PlayConditions.location(game, Keyword.UNDERGROUND))
                                action.appendEffect(
                                        new CheckPhaseLimitEffect(action, self, dwarf.getCardId()+"-dmg", 3, Phase.SKIRMISH,
                                                new AddUntilEndOfPhaseModifierEffect(
