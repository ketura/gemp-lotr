...
public class Card4_236 extends AbstractPermanent {
    public Card4_236() {
        super(Side.SHADOW, 1, CardType.CONDITION, Culture.RAIDER, "Howl of Harad");
    }

    @Override
    public List<OptionalTriggerAction> getOptionalAfterTriggers(String playerId, LotroGame game, EffectResult effectResult, PhysicalCard self) {
        if (game.getGameState().getTwilightPool() >= 4
                && TriggerConditions.losesSkirmishInvolving(game, effectResult, Filters.or(CardType.COMPANION, CardType.ALLY), Keyword.SOUTHRON)) {
