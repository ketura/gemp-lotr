...
public class Card1_339 extends AbstractSite {
    public Card1_339() {
        super("Frodo's Bedroom", SitesBlock.FELLOWSHIP, 3, 0, Direction.RIGHT);
    ...
    public List<? extends Action> getPhaseActionsInPlay(final String playerId, final LotroGame game, PhysicalCard self) {
        if (PlayConditions.canUseSiteDuringPhase(game, Phase.FELLOWSHIP, self)
                && Filters.filter(game.getGameState().getHand(playerId), game, Race.HOBBIT, Filters.playable(game)).size() > 0) {
