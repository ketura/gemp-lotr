...
public class FilterFactory {
    ...
    public FilterFactory() {
        ...
        simpleFilters.put("ring bearer", (actionContext, playerId, game, source, effectResult, effect) -> Filters.ringBearer);
        simpleFilters.put("any", (actionContext, playerId, game, source, effectResult, effect) -> Filters.any);
        simpleFilters.put("self", (actionContext, playerId, game, source, effectResult, effect) -> source);
        simpleFilters.put("your", (actionContext, playerId, game, source, effectResult, effect) -> Filters.owner(source.getOwner()));
        simpleFilters.put("weapon", (actionContext, playerId, game, source, effectResult, effect) -> Filters.weapon);
        ...
        simpleFilters.put("unbound",
                (actionContext, playerId, game, source, effectResult, effect) -> Filters.not(Keyword.RING_BOUND));
        ...
        parameterFilters.put("culture", (parameter, filterFactory) -> {
            ...
            return (actionContext, playerId, game, source, effectResult, effect) -> culture;
        });
        parameterFilters.put("side", (parameter, filterFactory) -> {
            ...
            return (actionContext, playerId, game, source, effectResult, effect) -> side;
        });
        parameterFilters.put("hasAttached",
                (parameter, filterFactory) -> {
                    ...
                    return (actionContext, playerId, game, source, effectResult, effect) -> Filters.hasAttached(filterableSource.getFilterable(actionContext, playerId, game, source, effectResult, effect));
                });
        parameterFilters.put("hasStacked",
                (parameter, filterFactory) -> {
                    final FilterableSource filterableSource = filterFactory.generateFilter(parameter);
                    return (actionContext, playerId, game, source, effectResult, effect) -> Filters.hasStacked(filterableSource.getFilterable(actionContext, playerId, game, source, effectResult, effect));
                });
        parameterFilters.put("hasStackedCount",
                (parameter, filterFactory) -> {
                    ...
                    return (actionContext, playerId, game, source, effectResult, effect) -> Filters.hasStacked(count, filterableSource.getFilterable(actionContext, playerId, game, source, effectResult, effect));
                });
        parameterFilters.put("attachedTo",
                (parameter, filterFactory) -> {
                    final FilterableSource filterableSource = filterFactory.generateFilter(parameter);
                    return (actionContext, playerId, game, source, effectResult, effect) -> Filters.attachedTo(filterableSource.getFilterable(actionContext, playerId, game, source, effectResult, effect));
                });
        parameterFilters.put("name",
                (parameter, filterFactory) -> (actionContext, playerId, game, source, effectResult, effect) -> Filters.name(parameter));
        parameterFilters.put("inSkirmishAgainst",
                (parameter, filterFactory) -> {
                    final FilterableSource filterableSource = filterFactory.generateFilter(parameter);
                    return (actionContext, playerId, game, source, effectResult, effect) -> Filters.inSkirmishAgainst(filterableSource.getFilterable(actionContext, playerId, game, source, effectResult, effect));
                });
        parameterFilters.put("assignedToSkirmish",
                (parameter, filterFactory) -> {
                    final FilterableSource filterableSource = filterFactory.generateFilter(parameter);
                    return (actionContext, playerId, game, source, effectResult, effect) -> Filters.assignedToSkirmishAgainst(filterableSource.getFilterable(actionContext, playerId, game, source, effectResult, effect));
                });
        parameterFilters.put("resistanceLessThan",
                (parameter, filterFactory) -> {
                    int amount = Integer.parseInt(parameter);
                    return (actionContext, playerId, game, source, effectResult, effect) -> Filters.maxResistance(amount + 1);
                });
        parameterFilters.put("not",
                (parameter, filterFactory) -> {
                    final FilterableSource filterableSource = filterFactory.generateFilter(parameter);
                    return (actionContext, playerId, game, source, effectResult, effect) -> Filters.not(filterableSource.getFilterable(actionContext, playerId, game, source, effectResult, effect));
                });
        parameterFilters.put("allyHome",
                (parameter, filterFactory) -> {
                    ...
                    return (actionContext, playerId, game, source, effectResult, effect) -> Filters.isAllyHome(number, sitesBlock);
                });
        parameterFilters.put("siteBlock",
                (parameter, filterFactory) -> {
                    final SitesBlock sitesBlock = Enum.valueOf(SitesBlock.class, parameter.toUpperCase().replace(' ', '_'));
                    return (actionContext, playerId, game, source, effectResult, effect) -> Filters.siteBlock(sitesBlock);
                });
        parameterFilters.put("siteNumber",
                (parameter, filterFactory) -> {
                    int number = Integer.parseInt(parameter);
                    return (actionContext, playerId, game, source, effectResult, effect) -> Filters.siteNumber(number);
                });
        parameterFilters.put("or",
                (parameter, filterFactory) -> {
                    ...
                    return (actionContext, playerId, game, source, effectResult, effect) -> {
                        Filterable[] filters1 = new Filterable[filterables.length];
                        for (int i = 0; i < filterables.length; i++)
                            filters1[i] = filterables[i].getFilterable(actionContext, playerId, game, source, effectResult, effect);
    ...
    private void appendFilter(Filterable value) {
        ...
        simpleFilters.put(filterName, (actionContext, playerId, game, source, effectResult, effect) -> value);
    }
    ...
    public FilterableSource generateFilter(String value) throws InvalidCardDefinitionException {
        ...
        if (filterStrings.length == 0)
            return (actionContext, playerId, game, source, effectResult, effect) -> Filters.any;
        ...
        return (actionContext, playerId, game, source, effectResult, effect) -> {
            ...
            for (int i = 0; i < filter.length; i++) {
                filter[i] = filters[i].getFilterable(actionContext, playerId, game, source, effectResult, effect);
            }
