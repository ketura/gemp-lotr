...
public class ArcheryGameProcess implements GameProcess {
    ...
    public GameProcess getNextProcess() {
        if (_game.getModifiersQuerying().shouldSkipPhase(_game.getGameState(), Phase.ARCHERY, null))
            return new CanSpotGameProcess(_game, CardType.MINION, new AssignmentGameProcess(_game), new RegroupGameProcess(_game));
                ...
                new PlayersPlayPhaseActionsInOrderGameProcess(_game, _game.getGameState().getPlayerOrder().getCounterClockwisePlayOrder(_game.getGameState().getCurrentPlayerId(), true), 0,
                        new ArcheryFireGameProcess(_game,
                                new EndOfPhaseGameProcess(_game, Phase.ARCHERY,
                                        new CanSpotGameProcess(_game, CardType.MINION, new AssignmentGameProcess(_game), new RegroupGameProcess(_game))))));
    }
}
