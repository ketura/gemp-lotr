...
public class Card3_086 extends AbstractMinion {
    ...
    public List<OptionalTriggerAction> getOptionalAfterTriggers(String playerId, final LotroGame game, EffectResult effectResult, PhysicalCard self) {
        if (PlayConditions.winsSkirmish(effectResult, self)
            ...
            action.appendEffect(
                    new ChooseActiveCardEffect(playerId, "Choose Blade Tip", Filters.name("Blade Tip"), Filters.zone(Zone.SHADOW_SUPPORT), Filters.owner(playerId)) {
                        @Override
                        protected void cardSelected(PhysicalCard card) {
                            action.insertEffect(
                                    new TransferPermanentEffect(card, Filters.findFirstActive(game.getGameState(), game.getModifiersQuerying(), Filters.keyword(Keyword.RING_BEARER))));
                        }
                    });
