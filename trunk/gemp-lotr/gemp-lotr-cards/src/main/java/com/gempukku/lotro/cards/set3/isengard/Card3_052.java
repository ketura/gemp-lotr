...
public class Card3_052 extends AbstractPermanent {
    ...
    protected List<? extends Action> getExtraPhaseActions(String playerId, LotroGame game, PhysicalCard self) {
        if (PlayConditions.canUseShadowCardDuringPhase(game.getGameState(), Phase.SHADOW, self, 0)
                ...
                && Filters.filter(game.getGameState().getDiscard(playerId), game.getGameState(), game.getModifiersQuerying(), Filters.culture(Culture.ISENGARD), Filters.keyword(Keyword.WEATHER), CardType.CONDITION, Filters.playable(game, -2)).size() > 0) {
            ...
            action.appendEffect(
                    new ChooseAndPlayCardFromDiscardEffect(playerId, discard, -2, Filters.and(Filters.culture(Culture.ISENGARD), Filters.keyword(Keyword.WEATHER), CardType.CONDITION)));
